# -*- coding: utf-8 -*-
"""build_r8_mode24.py — 由 SOXL 版三支 (TV-1M-*_(09月20日; 21.59)) 產出 R8 變體 (R7 + 模式 1/3 刪除, 只做多) (R2 只用模式 2 + 4, 再加 R3 的模式 2 進區方式 / 快慢線過濾 / 模式 4 可賣區):
...
import patch_r2_mode24, patch_r3_mode24, patch_r4_mode24, patch_r5_mode24, patch_r6_mode24, patch_r7_mode24, patch_r8_mode24
...
s = patch_r6_mode24.apply_main(s)
s = patch_r7_mode24.apply_main(s)
s = patch_r8_mode24.apply_main(s)

hdr = HEADER
hdr = rep(hdr, '主策略 (dashboard): 模式 1 MACD 柱到上下界 + 模式 2 RSI 14/28 可買區 + 模式 3 敏感 MACD 金叉 + 模式 4 EMA9 斜率向上',
               'R8 主策略 (dashboard): 只做多 (不做淡倉, 沒有任何 strategy.short); 只用 模式 2 RSI14 可買區 + 模式 4 EMA9 綠柱 決定買賣; 模式 1 柱到界 / 模式 3 金叉 已刪除 (資料不參與任何買賣決定, 預設不顯示); 買入後模式 4 出現第一根紅柱即以該根收盤價平倉 (M4紅柱)')
hdr = rep(hdr, '//  【本版規則 — 四個模式, 兩條合格條件】\n',
               '//  【R8 規則 — 只做多; 模式 1 / 3 已刪除 (④f 沒有加回開關, ④ / ④d 只剩純對照顯示, 預設關); 買 = 模式 2 可買區 (RSI14 到下界後轉勢) + 模式 4 綠柱, 本根至少一個剛出現; 賣 = 買入後模式 4 第一根紅柱 (斜率 ≤ 0) 本根收盤價立即平倉 M4紅柱 (排第一), 其次 可買區結束 / 15:58 收市】買 = 模式 2 處於可買區 且 RSI14 > RSI28 (④c 快慢線過濾, 預設開) 且 模式 4 EMA9 向上, 且本根至少一個剛出現 (剛進可買區 / RSI14 剛升穿 RSI28 / EMA9 剛轉向上)\n'
...
hdr = rep(hdr, 'S-M1&M2 (M1 賣訊在可賣區) / M2區結束 (可買區結束平倉)', 'S-M4區&M2 (預設: 模式 4 可賣區 + 模式 2 賣訊) 或 S-M2賣區 / S-M1&M2 (④f 選的賣法) / M2區結束 (可買區結束平倉) / M4紅柱 (R7: 紅柱那一根收盤價立即平倉, 排第一)')
...
hdr = rep(hdr, '版本戳 TV-1M-dashboard_(mode 1-4)_(MM月DD日; HH:MM)', '版本戳 TV-1M-dashboard_(mode 1-4)_R8_mode2_4only_(MM月DD日; HH:MM)')
hdr = rep(hdr, 'TV-1M-RSI-mode2', 'TV-1M-RSI-mode2_R8_mode2_4only', hdr.count('TV-1M-RSI-mode2'))
hdr = rep(hdr, 'TV-1M-winrate-mode4', 'TV-1M-winrate-mode4_R8_mode2_4only', hdr.count('TV-1M-winrate-mode4'))
n = hdr.count('\n') + s.count('\n') + 1 + 1
...
def pane(src, old_prefix, new_prefix, header_body):
    ...
    body = patch_r5_mode24.apply_rsi(patch_r4_mode24.apply_rsi(patch_r3_mode24.apply_rsi(body))) if 'RSI' in new_prefix else patch_r7_mode24.apply_m4(patch_r3_mode24.apply_m4(body))
...
rsi_hdr = '''R8 · 模式 2 (只用 RSI14 的可買區 / 可賣區) 專用副圖 · 配合主策略 TV-1M-dashboard_(mode 1-4)_R8_mode2_4only 一起掛 (R8 只做多, 買賣只用模式 2 + 4, 模式 1/3 已刪除)
...
m4_hdr = '''R8 · 模式 4 (EMA9 斜率) 專用副圖 · 配合主策略 TV-1M-dashboard_(mode 1-4)_R8_mode2_4only 一起掛 (R8 只做多, 買賣只用模式 2 + 4; 只有綠柱才可持倉, 買入後第一根紅柱即以該根收盤價平倉 M4紅柱)
模式 4 條件: EMA9 現值比 N 根前高 (預設 N = 1, ② 可改) = 趨勢向上 (綠柱) → 可以買入 / 可持倉; 斜率 ≤ 0 (紅柱) = 不可持倉: 主策略持倉中出現紅柱那一根立即以收盤價平倉 (M4紅柱)
顯示: 斜率柱 (% 表示; 綠 >0 可買 / 紅 ≤0 不可持倉) + 0 軸 + 底色; 由紅轉綠的那一根標「可買」, 由綠轉紅的那一根標「紅柱→平倉」; 右上統計窗內向上 K 數比例
...
for fn, html in ((f_main, 'paste_TV-1M-dashboard-R8.html'), (f_rsi, 'paste_TV-1M-RSI-mode2-R8.html'), (f_m4, 'paste_TV-1M-winrate-mode4-R8.html')):
    make_paste_page.make(fn, html)
    h = io.open(html, encoding='utf-8').read()
    h = h.replace('1 分鐘日內策略 (市場預設 美股, Inputs 可切港股)', 'R8 · 只做多 · 只用模式 2 + 4 (模式 1/3 已刪除; 紅柱立即平倉) · 1 分鐘日內策略 (市場預設 美股)')
