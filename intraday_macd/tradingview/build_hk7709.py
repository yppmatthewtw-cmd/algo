...
import patch_hk_tune
s = patch_hk_tune.apply(s)

hdr = HEADER
hdr = rep(hdr, '主策略 (dashboard): 模式 1 MACD 柱到上下界', '港股 7709 · 主策略 (dashboard): 模式 1 MACD 柱到上下界')
hdr = rep(hdr, '另有 可買區結束平倉 / 15:58 收市強平 / 可選固定止損\n', '另有 可買區結束平倉 / 15:58 收市強平 (半日市 11:58) / 可選 11:58 午休前平倉 / 可選 14:30 韓股收市平倉 / 可選固定止損 / 自動調參選到的追蹤止損或止賺 / 隔夜備援\n')
hdr = rep(hdr, '× 倍數 (買 1.5 / 賣 1.0; 可切手動);', '× 倍數 (買 1.5 / 賣 1.0; 可切手動; ⑨b 自動調參開啟時買方倍數 / 淺紅 N / 匹配窗口 / 出場模式逐日由 81 組中選);')
...
hdr = rep(hdr, '//  【共用規則】只做多 · 全部本金 · 收盤確認下一根開盤成交 · 手續費 0.03%/邊 + 滑點 2 tick · 美股 09:30-16:00 NY · 15:45 後不開新倉 · 15:58 強平\n',
                ...
                '//   港股 09:30-12:00, 13:00-16:00 HKT (⑤ 市場預設 港股, 有午休) · 午休前 11:50 起 / 14:30 韓股收市後 / 收市前 15:45 起不開新倉 · 15:58 強平 (12/24、12/31 半日市 11:58) · 午休前平倉 ⑤ 可選 (預設關 = 持倉過午休)\n')
hdr = rep(hdr, '//  【參數掃描 (⑨)】81 組 (買N×買k × 賣N×賣k, k∈1/1.5/2) 每一組都套上同一套四模式買 (M1&M3 匹配 + M2 可買區 + M4 向上) / M1 賣訊在可賣區 + 可買區結束平倉 的規則, 排名結果可直接填回 ④\n',
                '//  【參數掃描 (⑨)】81 組 (買N∈1/2/3 × 買k∈0.5/1/1.5 × M1/M3 匹配窗口∈5/10/20 × 出場模式∈現行/追蹤止損/止賺; 賣方 N/k 用 ④ 現行值) 每一組都套上同一套四模式買 / M1 賣訊在可賣區 + 可買區結束平倉 + EOD 的規則\n'
                '//  【後台自動調參 (⑨b, 本版預設開)】走動式: 每個交易日第一根, 從 81 組裡選「之前的交易日 (回看窗口逐日增長到 10 日)」報酬最高、至少 2 筆交易的組合, 套到今日真單 (由第二根 K 起; 無前視; 首 3 日用 ④/④c 手動值; 開啟時 ④b 校準不套用); 副圖參數表第 10 列與主圖掃描表底列顯示今日生效組合\n'
                '//  【韓股時段 (⑤, 本版預設開)】7709 追蹤 SK Hynix, 韓股 09:00-15:30 KST = 08:00-14:30 HKT; 14:30 後只剩莊家報價 → 不開新倉 (可關; 另可選 14:30 平倉)\n')
hdr = rep(hdr, 'EOD (15:58 強平) / 午休 (11:58 午休前平倉, ⑤ 可選, 預設關) / SL (止損) / 窗口結束', 'EOD (15:58 強平; 半日市 11:58) / 午休 (11:58 午休前平倉, ⑤ 可選, 預設關) / 韓股收市 (14:30, ⑤ 可選, 預設關) / SL (止損) / 追蹤止損 · 止賺 (自動調參選到該出場模式, 或 ⑨b 手動出場模式) / 隔夜備援 (留倉到翌日第一根平掉) / 窗口結束')
