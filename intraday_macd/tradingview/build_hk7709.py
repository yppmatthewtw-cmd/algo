...
hdr = rep(hdr, '//  【參數掃描 (⑨)】81 組 (買N×買k × 賣N×賣k, k∈1/1.5/2) 每一組都套上同一套四模式買 (M1&M3 匹配 + M2 可買區 + M4 向上) / M1 賣訊在可賣區 + 可買區結束平倉 的規則, 排名結果可直接填回 ④\n',
                ...
                '//  【後台自動調參 (⑨b, 本版預設開)】走動式: 每個交易日第一根, 從 81 組裡選「之前 N 日 (預設 10)」報酬最高、至少 M 筆 (預設 2) 交易的組合, 套到今日真單 (無前視; 初期日數不足用 ④/④c 手動值); 副圖參數表第 10 列與主圖掃描表底列顯示今日生效組合\n'
                '//  【韓股時段 (⑤, 本版預設開)】7709 追蹤 SK Hynix, 韓股 09:00-15:30 KST = 08:00-14:30 HKT; 14:30 後只剩莊家報價 → 不開新倉 (可關; 另可選 14:30 平倉)\n')
hdr = rep(hdr, 'EOD (15:58 強平) / 午休 (11:58 午休前平倉, ⑤ 可選, 預設關) / SL (止損) / 窗口結束', 'EOD (15:58 強平) / 午休 (11:58 午休前平倉, ⑤ 可選, 預設關) / 韓股收市 (14:30, ⑤ 可選, 預設關) / SL (止損) / 追蹤止損 · 止賺 (自動調參選到該出場模式時) / 窗口結束')
