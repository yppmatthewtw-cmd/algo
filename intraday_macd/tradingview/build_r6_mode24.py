# -*- coding: utf-8 -*-
"""build_r6_mode24.py — 由 SOXL 版三支 (TV-1M-*_(09月20日; 21.59)) 產出 R6 變體 (R5 + 模式 4 離開可買區即平倉) (R2 只用模式 2 + 4, 再加 R3 的模式 2 進區方式 / 快慢線過濾 / 模式 4 可賣區):
...
import patch_r2_mode24, patch_r3_mode24, patch_r4_mode24, patch_r5_mode24, patch_r6_mode24
...
hdr = rep(hdr, '主策略 (dashboard): 模式 1 MACD 柱到上下界 + 模式 2 RSI 14/28 可買區 + 模式 3 敏感 MACD 金叉 + 模式 4 EMA9 斜率向上',
               'R6 主策略 (dashboard): 只用 模式 2 RSI14 可買區 / 可賣區 + 模式 4 EMA9 斜率 決定買賣 (模式 1 柱到界 / 模式 3 金叉 預設關閉, ④f 可加回); R6 = R5 (模式 2 只用 RSI14) + 持倉中模式 4 離開可買區 (EMA9 剛轉向下) 即平倉 (M4區結束)')
hdr = rep(hdr, '//  【本版規則 — 四個模式, 兩條合格條件】\n',
               '//  【R6 規則 — 只用模式 2 + 4; 模式 2 只有 RSI14; R6 新增: 持倉中 EMA9 剛轉向下 (模式 4 離開可買區, 其它模式的買點失效) → 立即平倉 M4區結束 (④e 可關)】買 = 模式 2 處於可買區 且 RSI14 > RSI28 (④c 快慢線過濾, 預設開) 且 模式 4 EMA9 向上, 且本根至少一個剛出現 (剛進可買區 / RSI14 剛升穿 RSI28 / EMA9 剛轉向上)\n'
...
hdr = rep(hdr, 'S-M1&M2 (M1 賣訊在可賣區) / M2區結束 (可買區結束平倉)', 'S-M4區&M2 (預設: 模式 4 可賣區 + 模式 2 賣訊) 或 S-M2賣區 / S-M1&M2 (④f 選的賣法) / M2區結束 (可買區結束平倉) / M4區結束 (R6: EMA9 剛轉向下平倉)')
...
hdr = rep(hdr, '版本戳 TV-1M-dashboard_(mode 1-4)_(MM月DD日; HH:MM)', '版本戳 TV-1M-dashboard_(mode 1-4)_R6_mode2_4only_(MM月DD日; HH:MM)')
hdr = rep(hdr, 'TV-1M-RSI-mode2', 'TV-1M-RSI-mode2_R6_mode2_4only', hdr.count('TV-1M-RSI-mode2'))
hdr = rep(hdr, 'TV-1M-winrate-mode4', 'TV-1M-winrate-mode4_R6_mode2_4only', hdr.count('TV-1M-winrate-mode4'))
n = hdr.count('\n') + s.count('\n') + 1 + 1
...
rsi_hdr = '''R6 · 模式 2 (只用 RSI14 的可買區 / 可賣區) 專用副圖 · 配合主策略 TV-1M-dashboard_(mode 1-4)_R6_mode2_4only 一起掛 (R6 買賣只用模式 2 + 4)
...
m4_hdr = '''R6 · 模式 4 (EMA9 斜率) 專用副圖 · 配合主策略 TV-1M-dashboard_(mode 1-4)_R6_mode2_4only 一起掛 (R6 買賣只用模式 2 + 4; R6: 由可買轉可賣區的那一根「可賣」= 持倉即平倉 M4區結束)
...
for fn, html in ((f_main, 'paste_TV-1M-dashboard-R6.html'), (f_rsi, 'paste_TV-1M-RSI-mode2-R6.html'), (f_m4, 'paste_TV-1M-winrate-mode4-R6.html')):
    make_paste_page.make(fn, html)
    h = io.open(html, encoding='utf-8').read()
    h = h.replace('1 分鐘日內策略 (市場預設 美股, Inputs 可切港股)', 'R6 · 只用模式 2 + 4 決定買賣 (模式 2 只用 RSI14; 模式 4 離開可買區即平倉) · 1 分鐘日內策略 (市場預設 美股)')
