...
HEADER = """\
// ═══════════════════════════════════════════════════════════════════════════════
//  {name}  ·  主策略 (dashboard): 模式 1 MACD 柱動能減弱 + 模式 2 cRSI 轉勢 + 模式 3 敏感 MACD 金叉 + 模式 4 EMA9 斜率向上 · 1 分鐘 · 一個月回測 · Pine v6
...
//   模式 2 (cRSI 轉勢 + 動態上下界, ④c; 參考 whentotrade / Lars von Thienen 的 cRSI v4, 舊的 RSI(14) 門檻與 HL 結構已全部刪除): domcycle 20 → RSI 長度 10, vibration 10 平滑成 cRSI, 上下界 = 最近 40 根 cRSI 的第 10 / 90 百分位\n//       可買 (買區) = cRSI 梯度由 ≤0 轉為 >0 (本根高於前一根, 前一根不高於再前一根) 且 轉勢的谷底到達或低於下界; 可賣 (賣區) = 完全相反: 梯度由 ≥0 轉為 <0 且 峰頂到達或高於上界\n//       模式 2 的輸出只有「可買」/「可賣」兩種; 都是收盤確認的單根事件, 不重繪
...
//   3a 合格買入 = 模式 1、2、3 的買訊【同時】成立 (匹配窗口 ④c 預設 5 根) 且買入那一根模式 4 閘門為開 → 四模式同時
//   3b 合格賣出 = 模式 1 與 模式 2 的賣訊【同時】成立 (賣的匹配窗口 ④c 預設 5 根); 另有 15:58 收市強平 / 可選固定止損
//       ※ 模式 1 賣訊 (漲勢末段柱減弱) 與模式 2 的可賣 (cRSI 在上界轉勢向下) 都是漲勢末段訊號, 通常相距不遠; 若多數出場仍是收市強平, 放寬 ④c 的賣窗口。副圖參數表會顯示「賣: M1 幾筆 · M2 幾筆 → 同時幾筆」
//
//  【副圖分工】一支 Pine 腳本只能佔一個副圖。模式 1 與 3 (MACD 尺度) 在本腳本副圖; 模式 2 (cRSI) 在 TV-1M-RSI-mode2; 模式 4 (EMA9 斜率) 在 TV-1M-winrate-mode4; 三支一起 Add to chart, 同名設定要一致
//  【圖示】每個模式的每個買賣訊號 = 一條淺灰幼直線貫通主圖與副圖 (買 點線 / 賣 虛線, ⑧b 可關可調淡化); 模式 1 的 ▲ 淺藍買訊標在負柱底下, ▼ 淺紅賣訊標在正柱頂上;\n//        副圖只畫模式 3 的 9/26/9 快慢線 (12/26/9 的 DIF/DEA 線已移除, 柱保留給模式 1); 模式 3 的金叉 ▲ / 死叉 ▼ (只顯示) 標在 9/26/9 交叉點; 每條線左邊都有線名標籤 (擠在 0 軸附近的會自動上下拉開, 不再互相疊住)
