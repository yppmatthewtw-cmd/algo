...
HEADER = """\
// ═══════════════════════════════════════════════════════════════════════════════
//  {name}  ·  主策略 (dashboard): 模式 1 MACD 柱動能減弱 + 模式 2 RSI 超賣回歸 + 模式 3 敏感 MACD 金叉 + 模式 4 EMA9 斜率向上 · 1 分鐘 · 一個月回測 · Pine v6
...
//   模式 2 (RSI 超賣回歸 升級版, ④c): 超賣/超買門檻預設【動態】= 最近 120 根 RSI 的第 10 / 90 百分位 (消化自 cRSI; 固定 30 在 SOXL 1 分 K 幾乎碰不到);\n//       買訊 = RSI 上穿超賣界 且【結構過濾】通過: 這次超賣區的最低價高於上一次 (HL, 消化自 RSI Swing; 離開超賣區那一根定型, 不重繪); 可改成不過濾或再加 HH\n//       賣訊 = RSI 上穿 55 (S05 原版) 或 下穿超買界 (可選)
...
//
//  【副圖分工】一支 Pine 腳本只能佔一個副圖。模式 1 與 3 (MACD 尺度) 在本腳本副圖; 模式 2 (RSI) 在 TV-1M-RSI-mode2; 模式 4 (EMA9 斜率) 在 TV-1M-winrate-mode4; 三支一起 Add to chart, 同名設定要一致
