...
//
//  【圖示】每個模式的每個買賣訊號 = 一條淺灰幼直線貫通主圖與副圖 (買 點線 / 賣 虛線, ⑧b 可關可調淡化); 副圖最下方三條獨立訊號線: 模式 1 深綠 / 模式 2 深棕 (粗) / 模式 3 紫, 線上 ▲ 淺藍 = 該模式買訊, ▼ 淺紅 = 該模式賣訊;\n//        模式 3 的金叉 (○ 紫) / 死叉 (× 紅, 只顯示) 直接標在它自己的 DIF 9/26/9 線上; 模式 2 (RSI 與 MACD 不同尺度) 另有底部黃菱形; 每條線左邊都有線名標籤
