...
//
//  【副圖分工】一支 Pine 腳本只能佔一個副圖。模式 1 與模式 3 都是 MACD 數值 (同尺度) → 留在本腳本的副圖; 模式 2 是 RSI 0-100, 尺度不同 → 另一支 TV-1M-RSI-mode2 放它自己的副圖, 兩支一起 Add to chart
//  【圖示】每個模式的每個買賣訊號 = 一條淺灰幼直線貫通主圖與副圖 (買 點線 / 賣 虛線, ⑧b 可關可調淡化); 副圖最下方兩條獨立訊號線: 模式 1 深綠 / 模式 3 紫, 線上 ▲ 淺藍 = 買訊, ▼ 淺紅 = 賣訊;\n//        模式 3 的金叉 (○ 紫) / 死叉 (× 紅, 只顯示) 也標在它自己的 DIF 9/26/9 線上; 每條線左邊都有線名標籤 (擠在 0 軸附近的會自動上下拉開, 不再互相疊住)
//  【版面】回測摘要 / 觸發參數 / 參數掃描 三個表格全部靠右 (⑦⑨ 可改位置); 表格只能用 TradingView 的九宮格定位, 無法對齊自訂的垂直線
