# -*- coding: utf-8 -*-
"""build_r4_mode24.py — 由 SOXL 版三支 (TV-1M-*_(09月20日; 21.59)) 產出 R4 變體 (R3 + 模式 2 進區必須碰過界) (R2 只用模式 2 + 4, 再加 R3 的模式 2 進區方式 / 快慢線過濾 / 模式 4 可賣區):
...
import patch_r2_mode24, patch_r3_mode24, patch_r4_mode24
...
hdr = rep(hdr, '主策略 (dashboard): 模式 1 MACD 柱到上下界 + 模式 2 RSI 14/28 可買區 + 模式 3 敏感 MACD 金叉 + 模式 4 EMA9 斜率向上',
               'R4 主策略 (dashboard): 只用 模式 2 RSI 14/28 可買區 + 模式 4 EMA9 斜率 決定買賣 (模式 1 柱到界 / 模式 3 金叉 預設關閉, ④f 可加回); R4 = R3 (進區加「RSI14 升穿 RSI28」+ 快線在慢線之上 + 模式 4 紅柱 = 可賣區) + 進區前 20 根內 RSI14 必須碰過下界 / 上界 (沒碰過界的模式 2 買 / 賣點一律刪除)')
hdr = rep(hdr, '//  【本版規則 — 四個模式, 兩條合格條件】\n',
               '//  【R4 規則 — 只用模式 2 + 4; 模式 2 進區必須碰過界 (④c m2TouchLook, 預設 20 根內 RSI14 ≤ 下界 才可進可買區 / ≥ 上界 才可進可賣區)】買 = 模式 2 處於可買區 且 RSI14 > RSI28 (④c 快慢線過濾, 預設開) 且 模式 4 EMA9 向上, 且本根至少一個剛出現 (剛進可買區 / RSI14 剛升穿 RSI28 / EMA9 剛轉向上)\n'
...
hdr = rep(hdr, '版本戳 TV-1M-dashboard_(mode 1-4)_(MM月DD日; HH:MM)', '版本戳 TV-1M-dashboard_(mode 1-4)_R4_mode2_4only_(MM月DD日; HH:MM)')
hdr = rep(hdr, 'TV-1M-RSI-mode2', 'TV-1M-RSI-mode2_R4_mode2_4only', hdr.count('TV-1M-RSI-mode2'))
hdr = rep(hdr, 'TV-1M-winrate-mode4', 'TV-1M-winrate-mode4_R4_mode2_4only', hdr.count('TV-1M-winrate-mode4'))
n = hdr.count('\n') + s.count('\n') + 1 + 1
...
def pane(src, old_prefix, new_prefix, header_body):
    ...
    body = patch_r4_mode24.apply_rsi(patch_r3_mode24.apply_rsi(body)) if 'RSI' in new_prefix else patch_r3_mode24.apply_m4(body)
...
rsi_hdr = '''R4 · 模式 2 (RSI 14/28 可買區 / 可賣區) 專用副圖 · 配合主策略 TV-1M-dashboard_(mode 1-4)_R4_mode2_4only 一起掛 (R4 買賣只用模式 2 + 4)
R4: 進區前 N 根 (② 預設 20) 內 RSI14 必須碰過下界 (綠區) 才可進可買區 / 碰過上界 (紅區) 才可進可賣區; 沒碰過界的 ▲ / ▼ 一律不出現
...
m4_hdr = '''R4 · 模式 4 (EMA9 斜率) 專用副圖 · 配合主策略 TV-1M-dashboard_(mode 1-4)_R4_mode2_4only 一起掛 (R4 買賣只用模式 2 + 4)
...
for fn, html in ((f_main, 'paste_TV-1M-dashboard-R4.html'), (f_rsi, 'paste_TV-1M-RSI-mode2-R4.html'), (f_m4, 'paste_TV-1M-winrate-mode4-R4.html')):
    make_paste_page.make(fn, html)
    h = io.open(html, encoding='utf-8').read()
    h = h.replace('1 分鐘日內策略 (市場預設 美股, Inputs 可切港股)', 'R4 · 只用模式 2 + 4 決定買賣 (模式 2 進區必須碰過界) · 1 分鐘日內策略 (市場預設 美股)')
