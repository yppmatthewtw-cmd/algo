...
HEADER = """\
// ═══════════════════════════════════════════════════════════════════════════════
//  {name}  ·  MACD 柱動能減弱策略 · 1 分鐘回測版 (市場預設 美股, ⑤ 可一鍵切港股) · TradingView Pine Script v6
...
//
//  【本版關鍵參數 — 回測期間一年; 買賣變數與參數掃描表用來盡量捕獲期內最大昇幅】
//   ★ 回測期間 = 一年 (① 預設 方式 B 最近 365 天); 亦可改 方式 A 最近 N 個交易日 (預設 250) / 方式 C 指定起迄日期 — 全部在 Inputs 設定
//       ※ 1 分 K 一年約 98,000 根, TradingView 依方案只載入 5,000-20,000 根 → 實際回測從圖表最左那根開始, 報表標題顯示實際起始日
//       另有「策略設計的 K 線週期」欄 (預設 1 分鐘), 圖表週期與它不同時報表會警告; K 線週期本身在圖表上方切換, 策略永遠用圖表週期運算
//   ★ 觸發方式 = 動能減弱 N 根 (④)              ← 買: 負柱段連續 N 根「淺紅」(柱縮短) 完成; 賣: 正柱段連續 N 根「淺綠」完成; 可切回舊版 DIF/DEA 交叉
//   ★ 「是否儲夠下跌 / 上昇動能」= 買賣各自的變數 (④):
//       買: fadeBuy (淺紅 N, 預設 2) · kBuy (下跌動能門檻倍數, 0=不要求 / 1=自動門檻 / 越大越嚴, 預設 1) · mbBuy (負柱段最少根數, 預設 3)
//       賣: fadeSell (淺綠 N, 預設 2) · kSell (上昇動能門檻倍數, 預設 1) · mbSell (正柱段最少根數, 預設 3)
//       門檻 = 深度 ≥ 自動深度基準 × k, 面積 ≥ 深度門檻 × 根數 × 面積係數; 自動深度基準 = 近 250 根 |hist|/close 第 50 百分位 × 逐日校準係數
//   ★ 參數掃描表 (⑨, 主圖左下)                   ← 同窗口對 81 組 (買N∈1-3 × 買k∈0/1/2 × 賣N∈1-3 × 賣k∈0/1/2) 各跑虛擬回測,
//       同樣下一根開盤成交 / 時段 / 收市強平 / 手續費滑點 / ⑥ 固定止損, 按報酬與「捕獲 %」排名, 橙底 = 現行設定 → 把最佳組合填回 ④ 即套用下單
//   ★ margin_long = 0, margin_short = 0          ← Pine v6 預設 margin 100% 會令「全部本金」下單被拒 (0 交易)
//   ★ 自動校準 (④b) 本版預設關閉 (目標 = 0)   ← 2 日窗口用不到; 關閉時 k=1, 真實策略與掃描表用同一組門檻, 填回組合可完全重現掃描數字
//
//  規則: 買 = 負動能段「儲夠」後, 跌勢動能減低 (連續 N 根淺紅柱完成, 收盤確認) → 下一根開盤買; 賣 = 正動能段儲夠後, 升勢動能減低 (連續 N 根淺綠柱完成) → 下一根開盤賣
//        儲夠 = 同號柱連續根數 ≥ minBars 且 面積 Σ|hist|/close% ≥ minArea 且 深度 max|hist|/close% ≥ minDepth (④ reqMom 可關); 長倉 only
//  預設: 本金 100,000 · 最近 365 天 · 美股時段 09:30-16:00 America/New_York (DST 自動) · 15:45 後不開新倉 · 15:58 強制平倉
...
//        副圖狀態列只顯示策略名稱 (所有 input 與 plot 數值 display 設為不顯示); 橙框主圖/副圖共用同一組時間座標, 左右邊落在 K 與 K 之間
...
//  版本戳 TW-1M-MACD-Histogram-(MM月DD日_HH:MM), 括號內 = 建置時間 HKT; 檔名以 . 代替 : (Windows 檔名不接受冒號); 本檔零延續行, 每個語句自成一行
// ═══════════════════════════════════════════════════════════════════════════════
"""
