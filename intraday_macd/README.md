...
`tradingview/TV-1M-macd-hist,cross&rsi_(09月20日; 12.33).pine` — 由雙訊號版衍生, 加第三個買入模式, 賣出改為兩模式同時:
...
| 檔案 | 副圖內容 |
|:---|:---|
| `TV-1M-macd-hist,cross&rsi_(09月20日; 12.33).pine` | 主策略: DIF / DEA / MACD 柱 / 模式 3 的 DIF DEA + 模式 1 (深綠) 與模式 3 (紫) 兩條訊號線; 下單、回測報表、81 組掃描全部在這裡 |
| `TV-1M-RSI-mode2_(09月20日; 12.33).pine` | 模式 2 專用: RSI(14) 深棕粗線, ▲ 淺藍買 / ▼ 淺紅賣 直接標在曲線上, 含 30 / 55 / 70 門檻線與超買超賣色帶; 不下單 |
