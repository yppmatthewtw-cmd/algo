...
`tradingview/TV-1M-macd-hist,cross&rsi_(09月20日; 14.31).pine` — 由雙訊號版衍生, 加第三個買入模式, 賣出改為兩模式同時:
...
| 檔案 | 副圖內容 |
|:---|:---|
| `TV-1M-macd-hist,cross&rsi_(09月20日; 14.31).pine` | 主策略: DIF / DEA / MACD 柱 / 模式 3 的 DIF DEA + 模式 1 (深綠) 與模式 3 (紫) 兩條訊號線; 下單、回測報表、81 組掃描全部在這裡 |
...
| 腳本 | 內容 |
|:---|:---|
| `TV-1M-macd-hist,cross&rsi_(09月20日; 14.31).pine` | 主策略。④e 新增模式 4 閘門; 買入 = 模式 1、2、3 三訊號匹配 **且** 買入那一根模式 4 為開 → 四模式同時。賣出仍是 M1&M2 匹配, 模式 4 不參與賣出。81 組掃描同步套上閘門 |
| `TV-1M-RSI-mode2_(09月20日; 14.31).pine` | 模式 2 副圖 (不變, 只換版本戳) |
| `TV-1M-winrate-mode4_(09月20日; 14.31).pine` | 模式 4 副圖: 階梯線 1 = 可開倉 (綠底) / 0.5 = 可做的日子但不在時段 (灰底) / 0 = 開市向下不做 (紅底); 每天開市一個標籤寫當日判定; 右上統計 |
