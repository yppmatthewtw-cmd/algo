...
## R7 · 模式 4 只有綠柱才可持倉, 紅柱那一根立即平倉 (TV-1M-*_R7_mode2_4only_, 2026-09-26 22:25)

`tradingview/patch_r7_mode24.py` (`build_r7_mode24.py`), 取代 R6 的「剛轉向下才平倉、下一根開盤成交」: 持倉中只要本根 EMA9 斜率 ≤ 0 (副圖紅柱) → `strategy.close(immediately = true)` 本根收盤價成交, 出場原因 M4紅柱, 排在所有出場之前 (剛在本根開盤買入、本根即轉紅也會被平掉)。⑨ 掃描同步 (該組的 EMA9 本根不向上 → 以本根收盤價含滑點立即平倉)。模式 4 副圖: 轉紅那一根標「紅柱→平倉」, 表格顯示「不可持倉 (立即平倉)」。
